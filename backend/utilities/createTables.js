...
// IF NOT EXISTS
        this.sql = {
            ...
            borrow: "CREATE TABLE IF NOT EXISTS BORROW (member_id INT NOT NULL, book_id INT NOT NULL, borrow_date DATE DEFAULT CURRENT_DATE(), return_date DATE, status ENUM('Borrowed', 'Returned', 'Overdue') DEFAULT 'Borrowed', FOREIGN KEY (member_id) REFERENCES MEMBER(member_id), FOREIGN KEY (book_id) REFERENCES BOOK(book_id));",
            bill: "CREATE TABLE IF NOT EXISTS BILL (bill_id INT PRIMARY KEY AUTO_INCREMENT, member_id INT NOT NULL, issue_date DATE DEFAULT CURRENT_DATE(), amount DECIMAL(10,2) NOT NULL, bill_type ENUM('Fine', 'Membership', 'Reservation', 'Service', 'Other') NOT NULL, description VARCHAR(255), payment_status ENUM('Pending', 'Paid', 'Cancelled') DEFAULT 'Pending', FOREIGN KEY (member_id) REFERENCES MEMBER(member_id));"
        }; 
